...
const { encrypt, decrypt } = require('../services/cryptoService'); // <-- ADD THIS LINE
...
exports.addKey = async (req, res) => {
  ...
  try {
    const encryptedKey = encrypt(apiKey); // <-- ADD THIS LINE
    await pool.query(
      ...
      [req.user.id, provider, keyName, encryptedKey] // <-- USE encryptedKey HERE
    );
    res.json({ ok: true });
  } catch (err) {
    console.error(err);
...
// ... (getKeys, deleteKey, reactivateKey, deactivateKey are unchanged) ...
exports.getKeys = async (req, res) => {
  // This function only retrieves metadata, so no changes are needed.
...
exports.testKey = async (req, res) => {
    const id = req.params.id;
    try {
        // We need to select the api_key itself now
        ...
        const decryptedKey = decrypt(keyToTest.api_key); // <-- DECRYPT THE KEY
        ...
        if (provider === 'gemini') {
            testUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent?key=${decryptedKey}`; // <-- USE decryptedKey
            ...
            headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${decryptedKey}` }; // <-- USE decryptedKey
